...
function getAllMonth(date){
	$.ajax({
		url : ctxRoot + "/data/getBuyerAllMonth",
		type : 'post',
		data : {
			"reportType":"day"
		}, 
...
function getChartData(reportValue){	
		...
		$.ajax({
			...
			dataType: "json",
			success	: function(data){		
				if(data.list != null && data.model != null){
					...
					$('#aprChartNone').addClass("hidden");
					$('#dpeChartNone').addClass("hidden");
					$('#DailyTurnoverChartNone').addClass("hidden");
...
function getBuyerChartData(list,reportValue){
	var reportDates=""; 
...
function parseDailyChart(list){
	...
	$('#DailyTurnoverChartNone').removeClass("hidden");
	$('#dpeChartNone').removeClass("hidden");
	$('#aprChartNone').removeClass("hidden");
	var months=[];
	var myAPR=[];
	var category1=[];
	var category2=[];
