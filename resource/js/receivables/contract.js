...
function getContractTotal(){
	$.ajax({
		url : ctxRoot + "/contract/getSupplierContractTotal",
		type: 'POST',  
		data : {}, 
		dataType: "json",
...
function loadContractList(){
	var params = {"isAjax":"y", "isEasyUI":"1", "type":"supplier"};
	...
	$('#contractList').datagrid({
		...
		columns:[[
		   ...
		   {field:'buyerName', title:'买方名称',  align:'left', width:'25%', sortable:true, formatter:getBname},
...
function getContractNo(val, row){
	return "<span class='color-minblue changecolorblue link' title='"+row.contractNo+"' onclick=\"openUrl(\'/contract/receivables/getDetail?id="+row.id+"\');\">"+row.contractNo+"</span>";
}
